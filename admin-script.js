...
function loadBookings() {
    ...
    try {
        ...
        // Debug: Log each booking to see if package information is present
        bookings.forEach((booking, index) => {
            console.log(`Booking ${index + 1}:`, booking);
            console.log(`Booking ${index + 1} package:`, booking.selectedPackage);
        });
...
function updateSlotsGrid() {
    ...
    const now = new Date();
    const isToday = overviewDate === now.toISOString().split('T')[0];

    // Helper to get end time in 24h format from slot string
    function getSlotEndTime(slot) {
        const end = slot.split('-')[1].trim();
        const [time, meridian] = end.split(' ');
    ...
    const filteredSlots = timeSlots.filter(slot => {
        ...
        const { hours, minutes } = getSlotEndTime(slot);
    ...
    slotsGrid.innerHTML = filteredSlots.map(time => {
            ...
            return `
                <div class="slot-item booked">
                    ...
                    <div class="slot-info">
                        ...
                        <div class="package-info">₹ ${booking.selectedPackage || 'Not Selected'}</div>
                    </div>
    ...
    bookingsContainer.innerHTML = bookingsToShow.map(booking => `
        <div class="booking-card ${booking.status}">
            ...
            <div class="booking-details">
                ...
                <div class="detail-row">
                    <div class="detail-item">
                        <strong>Event:</strong> ${booking.type}
                    </div>
                    <div class="detail-item">
                        <strong>Package:</strong> ${booking.selectedPackage || 'Not Selected'}
                    </div>
                </div>
                <div class="detail-row">
                    <div class="detail-item">
            ...
            <div class="booking-actions">
                ...
                <button onclick="deleteBooking('${booking.id}')" class="btn btn-danger">
    ...
    bookingDetails.innerHTML = `
        <div class="booking-details-modal">
            ...
            <div class="detail-section">
                ...
                <p><strong>Package:</strong> ${booking.selectedPackage || 'Not Selected'}</p>
            </div>
...
function openConfirmModal(bookingId) {
    ...
    // Debug: Log the booking object to see what data is available
...
function deleteBooking(bookingId) {
    ...
    // Get the booking details before deletion for logging
    const deletedBooking = bookings[bookingIndex];
    console.log('Deleting booking details:', deletedBooking);
    
    // Remove booking from array permanently
    bookings.splice(bookingIndex, 1);
    
    // Update localStorage to permanently remove the booking
    localStorage.setItem('bookings', JSON.stringify(bookings));
    
    // Verify deletion by checking localStorage
    const updatedBookings = JSON.parse(localStorage.getItem('bookings') || '[]');
    const bookingStillExists = updatedBookings.find(b => b.id === bookingId);
    ...
    console.log('Booking permanently deleted from localStorage');
    ...
    showNotification('Booking deleted permanently!', 'success');
}
